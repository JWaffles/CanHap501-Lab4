/**
 **********************************************************************************************************************
 * @file       CanHap501_Lab4.pde
 * @author     Jérémy Durette
 * @version    V1.0.0
 * @date       18-February-2022
 * @brief      CanHap501 Lab 4, playing with the forcefeedback of the Haply

 ** NOTE: This code was built upon the code of the sketch_2_Hello_Wall.pde listed below.
 **********************************************************************************************************************
...
void setup(){
  ...
  cp5.addSlider("looptime")
      ...
      ;
  cp5.addButton("SquarePathing")
      .setValue(0)
      .setPosition(10, 515)
      .setSize(200, 40)
      ;
  cp5.addButton("CircularPathing")
      .setValue(0)
      .setPosition(10, 560)
      .setSize(200, 40)
      ;
...
public void SquarePathing(int theValue) {
  println("SquarePathing");
  pathing = 1;
  pathingDirection = 0;
  px = max_left;
  py = max_top;
...
public void CircularPathing(int theValue) {
  println("CircularPathing");
  pathing = 2;
  pathingDirection = 0;
  px = max_left + length_x / 2;
  py = max_top;
}
...
void draw() {
  ...
  if(renderingForce == false) {
    background(255);
    if(pathing == 1) {
      update_square_pathing();
    } else if(pathing == 2){
      update_circular_pathing();
    }
...
void update_circular_pathing(){
  pa = pa + increment/100;
  
  px = (float) Math.cos(pa) * length_x/2 + length_x/2;
  py = (float) Math.sin(pa) * length_y/2;
...
void update_square_pathing(){
  switch(pathingDirection) {
    case 0 : // Right
      px = px + increment;
      py = max_top;
      if(px >= max_right) {
